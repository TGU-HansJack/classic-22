...
HTML;
    ...
    $aiSecurityEnabled = new \Typecho\Widget\Helper\Form\Element\Select(
        'aiSecurityEnabled',
        [
            '1' => _t('开启'),
            '0' => _t('关闭'),
        ...
        '1',
        _t('AI 安全防护'),
        _t('建议开启：启用接口令牌校验、同源校验与请求频控，降低爬取和恶意滥用风险。')
    );
    $form->addInput($aiSecurityEnabled);

    $aiChatRatePerMinute = new \Typecho\Widget\Helper\Form\Element\Text(
        'aiChatRatePerMinute',
        null,
        '12',
        _t('AI 对话每分钟限额（单 IP）'),
        _t('建议 6~30；超限后会返回“请求过于频繁”。')
    );
    $form->addInput($aiChatRatePerMinute);

    $aiArticlesRatePerMinute = new \Typecho\Widget\Helper\Form\Element\Text(
        'aiArticlesRatePerMinute',
        null,
        '40',
        _t('文章列表接口每分钟限额（单 IP）'),
        _t('用于限制文章下拉接口被高频爬取。')
    );
    $form->addInput($aiArticlesRatePerMinute);

    $aiGuardTokenTtl = new \Typecho\Widget\Helper\Form\Element\Text(
        'aiGuardTokenTtl',
        null,
        '7200',
        _t('AI 安全令牌有效期（秒）'),
        _t('默认 7200 秒（2 小时）。到期后刷新页面可自动续期。')
    );
    $form->addInput($aiGuardTokenTtl);
...
function classic22AiBuildBootstrapToken($archive): string
{
    ...
    if (!classic22AiSecurityEnabled($archive->options)) {
        return '';
    }

    return classic22AiBuildGuardToken($archive);
}

function classic22AiSecurityEnabled($options): bool
{
    return classic22LinuxDoGetOption($options, 'aiSecurityEnabled', '1') !== '0';
}

function classic22AiRateLimitValue($options, string $key, int $default, int $min, int $max): int
{
    $raw = trim((string) classic22LinuxDoGetOption($options, $key, (string) $default));
    $value = (int) $raw;
    if ($value <= 0) {
        $value = $default;
    }
...
function classic22AiGuardTokenTtl($options): int
{
    return classic22AiRateLimitValue($options, 'aiGuardTokenTtl', 7200, 300, 86400);
}

function classic22AiChatRateLimit($options): int
{
    return classic22AiRateLimitValue($options, 'aiChatRatePerMinute', 12, 1, 300);
}

function classic22AiArticlesRateLimit($options): int
{
    return classic22AiRateLimitValue($options, 'aiArticlesRatePerMinute', 40, 1, 600);
}

function classic22AiRequestMethod(): string
{
    return strtoupper(trim((string) ($_SERVER['REQUEST_METHOD'] ?? 'GET')));
}

function classic22AiIsSameOriginRequest($archive): bool
{
    ...
    $siteBaseUrl = classic22LinuxDoSiteBaseUrl($archive->options);
    $siteParts = parse_url($siteBaseUrl);
    ...
    $matchesSource = static function (string $url) use ($siteScheme, $siteHost, $sitePort): bool {
        if ($url === '') {
            return true;
        }
        ...
        if ($scheme !== $siteScheme || $host !== $siteHost) {
            return false;
        }

        if ($sitePort > 0 && $port > 0 && $sitePort !== $port) {
            return false;
        }
    ...
    return $matchesSource($origin) && $matchesSource($referer);
}

function classic22AiClientFingerprint($archive): string
{
    $ipCandidates = [];

    $forwardedFor = trim((string) ($_SERVER['HTTP_X_FORWARDED_FOR'] ?? ''));
    if ($forwardedFor !== '') {
        foreach (explode(',', $forwardedFor) as $segment) {
            $segment = trim($segment);
            if ($segment !== '') {
                $ipCandidates[] = $segment;
            }
        }
    }
...
function classic22AiRateLimitStoragePath(): string
{
    $base = rtrim((string) __TYPECHO_ROOT_DIR__, '/\\');
    return $base . '/var/classic22-ai-rate-limit.json';
}

function classic22AiRateLimitCheck($archive, string $bucket, int $limit, int $windowSeconds = 60): array
{
    if ($limit <= 0) {
        return ['ok' => true, 'remaining' => 0, 'retryAfter' => 0];
    }

    $path = classic22AiRateLimitStoragePath();
    $dir = dirname($path);
    if (!is_dir($dir)) {
        @mkdir($dir, 0755, true);
    }
    ...
    $fingerprint = classic22AiClientFingerprint($archive);
    $scopeKey = $bucket . ':' . $fingerprint;
    ...
    $entry = [];
    if (isset($store[$scopeKey]) && is_array($store[$scopeKey])) {
        $entry = $store[$scopeKey];
    }
    ...
    if (isset($entry['hits']) && is_array($entry['hits'])) {
        foreach ($entry['hits'] as $timestamp) {
            $stamp = (int) $timestamp;
            if ($stamp >= $windowStart && $stamp <= $now) {
                $hits[] = $stamp;
            }
        }
    }
    ...
    foreach ($store as $key => $item) {
        ...
        $updatedAt = (int) ($item['updatedAt'] ?? 0);
        if ($updatedAt > 0 && $updatedAt < ($now - 86400)) {
            unset($store[$key]);
        }
    }
...
function classic22AiSecretKey($archive): string
{
    $siteUrl = classic22LinuxDoSiteBaseUrl($archive->options);
    $apiKey = trim((string) classic22LinuxDoGetOption($archive->options, 'aiApiKey', ''));
    $fallback = __TYPECHO_ROOT_DIR__ . '|' . __DIR__;
    $seed = $apiKey !== '' ? ($siteUrl . '|' . $apiKey) : ($siteUrl . '|' . $fallback);

    return hash('sha256', $seed);
}

function classic22AiBuildGuardToken($archive): string
{
    $ts = time();
    $nonce = classic22LinuxDoGenerateState();
    $fingerprint = classic22AiClientFingerprint($archive);
    $payload = $ts . '|' . $nonce . '|' . $fingerprint;
    $mac = hash_hmac('sha256', $payload, classic22AiSecretKey($archive));

    return base64_encode($ts . '.' . $nonce . '.' . $mac);
}

function classic22AiVerifyGuardToken($archive, string $token): bool
{
    $token = trim($token);
    if ($token === '') {
        return false;
    ...
    $ttl = classic22AiGuardTokenTtl($archive->options);
    $now = time();
    if ($ts > $now + 60 || ($now - $ts) > $ttl) {
        return false;
    }

    $fingerprint = classic22AiClientFingerprint($archive);
    $payload = $ts . '|' . $nonce . '|' . $fingerprint;
    $expected = hash_hmac('sha256', $payload, classic22AiSecretKey($archive));

    return hash_equals($expected, $sig);
}

function classic22AiExtractGuardTokenFromRequest($archive): string
{
    $header = trim((string) ($_SERVER['HTTP_X_CLASSIC22_AI_TOKEN'] ?? ''));
    if ($header !== '') {
        return $header;
    }

    $token = trim((string) $archive->request->get('aiToken'));
    if ($token !== '') {
        return $token;
    }
    ...
    return trim((string) ($decoded['aiToken'] ?? ''));
}

function classic22AiSecurityChecks($archive, string $action): array
{
    if (!classic22AiSecurityEnabled($archive->options)) {
        return ['ok' => true];
    }

    $method = classic22AiRequestMethod();
    if ($action === 'chat' && $method !== 'POST') {
        return ['ok' => false, 'status' => 405, 'message' => '请求方法不允许。'];
    }

    if ($action === 'articles' && !in_array($method, ['GET', 'POST'], true)) {
        return ['ok' => false, 'status' => 405, 'message' => '请求方法不允许。'];
    }

    if (!classic22AiIsSameOriginRequest($archive)) {
        return ['ok' => false, 'status' => 403, 'message' => '来源校验失败，请从本站页面发起请求。'];
    }

    $token = classic22AiExtractGuardTokenFromRequest($archive);
    if (!classic22AiVerifyGuardToken($archive, $token)) {
        return ['ok' => false, 'status' => 403, 'message' => '请求令牌无效，请刷新页面后重试。'];
    }

    $limit = $action === 'chat'
        ? classic22AiChatRateLimit($archive->options)
        : classic22AiArticlesRateLimit($archive->options);

    $limitResult = classic22AiRateLimitCheck($archive, 'ai:' . $action, $limit, 60);
    if (empty($limitResult['ok'])) {
        return [
            'ok' => false,
            'status' => 429,
            'message' => '请求过于频繁，请稍后再试。',
            'retryAfter' => (int) ($limitResult['retryAfter'] ?? 60),
        ];
    }
...
function classic22AiHandleRequest($archive): void
{
    ...
    $securityResult = classic22AiSecurityChecks($archive, $action);
    if (empty($securityResult['ok'])) {
        if (!headers_sent() && isset($securityResult['retryAfter']) && (int) $securityResult['retryAfter'] > 0) {
            header('Retry-After: ' . (int) $securityResult['retryAfter']);
        }

        classic22AiSendJson([
            'ok' => false,
            'message' => (string) ($securityResult['message'] ?? '请求已被安全策略拦截。'),
        ], (int) ($securityResult['status'] ?? 403));
    }
