...
HTML;
    ...
    $liveWsHost = new \Typecho\Widget\Helper\Form\Element\Text(
        'liveWsHost',
        null,
        '127.0.0.1',
        _t('内置 WS 监听 Host'),
        _t('仅用于 ws-server.php 脚本，默认 127.0.0.1')
    );
    $form->addInput($liveWsHost);

    $liveWsPort = new \Typecho\Widget\Helper\Form\Element\Text(
        'liveWsPort',
        null,
        '9527',
        _t('内置 WS 监听 Port'),
        _t('仅用于 ws-server.php 脚本，默认 9527')
    );
    $form->addInput($liveWsPort);
...
{
    $keys = ['linuxDoClientId', 'linuxDoClientSecret', 'liveWsEnabled', 'liveWsEndpoint', 'liveWsHost', 'liveWsPort'];
    $extracted = [];
...
function classic22LiveOptionName(): string
{
    return 'classic22_live_presence';
}

function classic22LiveNormalizePath(string $raw): string
{
    $path = trim($raw);
    if ($path === '') {
        return '/';
...
function classic22LiveClientId(): string
{
    $cookieName = '__classic22_live_sid';
    $sid = trim((string) ($_COOKIE[$cookieName] ?? ''));

    if (!preg_match('/^[a-f0-9]{24,64}$/', $sid)) {
        try {
            $sid = bin2hex(random_bytes(16));
        } catch (\Throwable $exception) {
            $sid = sha1(uniqid('', true) . mt_rand());
        }
        ...
        if (PHP_VERSION_ID >= 70300) {
            ...
            setcookie($cookieName, $sid, time() + 31536000, '/', '', $secure, true);
        }
...
function classic22LiveLoadState(): array
{
    $state = classic22LinuxDoLoadOptionJson(classic22LiveOptionName());
...
function classic22LiveSaveState(array $state): void
{
    if (!isset($state['clients']) || !is_array($state['clients'])) {
        $state['clients'] = [];
    }

    classic22LinuxDoSaveOptionJson(classic22LiveOptionName(), $state);
}

function classic22LivePruneState(array $state, int $now, int $ttl = 90): array
{
    $clients = isset($state['clients']) && is_array($state['clients']) ? $state['clients'] : [];
    ...
    foreach ($clients as $sid => $client) {
        ...
        $updated = (int) ($client['updated'] ?? 0);
        if ($updated <= 0 || ($now - $updated) > $ttl) {
            unset($clients[$sid]);
            continue;
        }

        $clients[$sid]['path'] = classic22LiveNormalizePath((string) ($client['path'] ?? '/'));
        $clients[$sid]['updated'] = $updated;
    }
...
function classic22LiveBuildCounts(array $state): array
{
    $counts = [];
    $clients = isset($state['clients']) && is_array($state['clients']) ? $state['clients'] : [];
...
function classic22LiveJsonResponse(array $payload): void
{
...
function classic22LiveBinaryResponse(string $payload): void
{
...
function classic22LiveWsOptionName(): string
{
    return 'classic22_live_ws_sessions';
}

function classic22LiveWsLoadSessions(): array
{
    $state = classic22LinuxDoLoadOptionJson(classic22LiveWsOptionName());
...
function classic22LiveWsSaveSessions(array $state): void
{
    if (!isset($state['sessions']) || !is_array($state['sessions'])) {
        $state['sessions'] = [];
    }

    classic22LinuxDoSaveOptionJson(classic22LiveWsOptionName(), $state);
}

function classic22LiveWsRandomHex(int $bytes = 16): string
{
    try {
...
function classic22LiveWsBase64Encode(string $binary): string
{
    return rtrim(strtr(base64_encode($binary), '+/', '-_'), '=');
}

function classic22LiveWsBase64Decode(string $raw): string
{
    $text = strtr($raw, '-_', '+/');
...
function classic22LiveWsBuildAccept(string $secKey): string
{
    return base64_encode(sha1($secKey . '258EAFA5-E914-47DA-95CA-C5AB0DC85B11', true));
}

function classic22LiveWsPackFrameText(string $payload): string
{
    $len = strlen($payload);
    $frame = chr(0x81);
...
function classic22LiveWsUnpackFrames(string $raw): array
{
    $frames = [];
    $offset = 0;
    $total = strlen($raw);
    ...
    while ($offset + 2 <= $total) {
        ...
        if ($len === 126) {
            ...
            $len = unpack('n', substr($raw, $offset + 2, 2))[1];
            $header = 4;
        } elseif ($len === 127) {
            if ($offset + 10 > $total) {
                break;
            }
            $parts = unpack('Nhigh/Nlow', substr($raw, $offset + 2, 8));
        ...
        $mask = $masked ? substr($raw, $offset + $header, 4) : '';
        $payload = substr($raw, $offset + $header + $maskLen, $len);
...
function classic22LiveWsClientIp(): string
{
    $ip = trim((string) ($_SERVER['REMOTE_ADDR'] ?? ''));
    return $ip !== '' ? $ip : '0.0.0.0';
}

function classic22LiveWsPruneSessions(array $state, int $now, int $ttl = 120): array
{
...
function classic22LiveWsCountsFromSessions(array $state): array
{
    $counts = [];
    $sessions = isset($state['sessions']) && is_array($state['sessions']) ? $state['sessions'] : [];
    ...
    foreach ($sessions as $session) {
        ...
        $path = classic22LiveNormalizePath((string) ($session['path'] ?? '/'));
        if (!isset($counts[$path])) {
            $counts[$path] = 0;
        }
        $counts[$path]++;
...
function classic22LiveWsBuildBroadcastFrames(array $counts): string
{
    $out = '';
    ...
    foreach ($counts as $path => $count) {
        ...
        if (is_string($payload) && $payload !== '') {
            $out .= classic22LiveWsPackFrameText($payload);
        }
    }
...
function classic22LiveWsHandleOpen($archive): void
{
    $secKey = trim((string) $archive->request->get('ws_key'));
    if ($secKey === '') {
        classic22LiveJsonResponse([
            'ok' => 0,
            'message' => 'Missing ws_key',
        ]);
    }

    $now = time();
    $sid = classic22LiveWsRandomHex(16);
    $ip = classic22LiveWsClientIp();

    $state = classic22LiveWsLoadSessions();
    $state = classic22LiveWsPruneSessions($state, $now);
    ...
    classic22LiveWsSaveSessions($state);

    classic22LiveJsonResponse([
        'ok' => 1,
        'sid' => $sid,
        'accept' => classic22LiveWsBuildAccept($secKey),
        'transport' => 'websocket',
    ]);
}

function classic22LiveWsHandleSend($archive): void
{
    $sid = trim((string) $archive->request->get('sid'));
    if ($sid === '') {
        classic22LiveJsonResponse(['ok' => 0, 'message' => 'Missing sid']);
    }
    ...
    if (!isset($state['sessions'][$sid]) || !is_array($state['sessions'][$sid])) {
        classic22LiveJsonResponse(['ok' => 0, 'message' => 'Session not found']);
    }

    $remoteIp = classic22LiveWsClientIp();
    $savedIp = (string) ($state['sessions'][$sid]['ip'] ?? '');
    if ($savedIp !== '' && $savedIp !== $remoteIp) {
        classic22LiveJsonResponse(['ok' => 0, 'message' => 'Session IP mismatch']);
    }

    $raw = (string) file_get_contents('php://input');
    $frames = classic22LiveWsUnpackFrames($raw);
    $path = classic22LiveNormalizePath((string) ($state['sessions'][$sid]['path'] ?? '/'));
    ...
    foreach ($frames as $frame) {
        ...
        if ($opcode === 0x1) {
            ...
            if (is_array($decoded) && (($decoded['type'] ?? '') === 'subscribe')) {
                $path = classic22LiveNormalizePath((string) ($decoded['path'] ?? $decoded['page'] ?? '/'));
            }
        }
    }

    $state['sessions'][$sid]['path'] = $path;
    $state['sessions'][$sid]['updated'] = $now;

    $counts = classic22LiveWsCountsFromSessions($state);
    $broadcastFrames = classic22LiveWsBuildBroadcastFrames($counts);

    if ($broadcastFrames !== '') {
        foreach ($state['sessions'] as $otherSid => $session) {
            if (!is_array($session)) {
                continue;
            }

            $existingQueue = classic22LiveWsBase64Decode((string) ($session['queue'] ?? ''));
            $newQueue = $existingQueue . $broadcastFrames;
            $state['sessions'][$otherSid]['queue'] = classic22LiveWsBase64Encode($newQueue);
            $state['sessions'][$otherSid]['updated'] = $now;
        }
    }
...
function classic22LiveWsHandlePoll($archive): void
{
    $sid = trim((string) $archive->request->get('sid'));
    if ($sid === '') {
        classic22LiveBinaryResponse('');
    }
    ...
    $queue = classic22LiveWsBase64Decode((string) ($state['sessions'][$sid]['queue'] ?? ''));
    $state['sessions'][$sid]['queue'] = '';
    $state['sessions'][$sid]['updated'] = $now;
    classic22LiveWsSaveSessions($state);

    classic22LiveBinaryResponse($queue);
}

function classic22LiveWsHandleClose($archive): void
{
    $sid = trim((string) $archive->request->get('sid'));
    $state = classic22LiveWsLoadSessions();

    if ($sid !== '' && isset($state['sessions'][$sid])) {
        unset($state['sessions'][$sid]);
        classic22LiveWsSaveSessions($state);
    }
...
function classic22LiveHandlePing($archive): void
{
    $now = time();
    $sid = classic22LiveClientId();

    $path = trim((string) $archive->request->get('path'));
    if ($path === '') {
        $path = (string) (parse_url((string) $archive->request->getRequestUri(), PHP_URL_PATH) ?? '/');
    }
    $path = classic22LiveNormalizePath($path);

    $state = classic22LiveLoadState();
    $state = classic22LivePruneState($state, $now);
    $state['clients'][$sid] = [
        'path' => $path,
        'updated' => $now,
    ];

    classic22LiveSaveState($state);

    $counts = classic22LiveBuildCounts($state);
    classic22LiveJsonResponse([
        'ok' => 1,
        'transport' => 'theme-live',
        'path' => $path,
...
function classic22LiveHandlePoll($archive): void
{
    $now = time();
    ...
    $counts = classic22LiveBuildCounts($state);
    $requested = trim((string) $archive->request->get('paths'));
    ...
    $result = [];
    foreach (explode(',', $requested) as $rawPath) {
        $path = classic22LiveNormalizePath((string) $rawPath);
        if (isset($result[$path])) {
            continue;
        }
        $result[$path] = (int) ($counts[$path] ?? 0);
    }
...
function classic22LiveHandleRequest($archive): void
{
    if (!is_object($archive)) {
        return;
    }

    $action = trim((string) $archive->request->get('live_action'));
    if ($action === '') {
        return;
    }

    if ($action === 'ping') {
        classic22LiveHandlePing($archive);
    }

    if ($action === 'poll') {
        classic22LiveHandlePoll($archive);
    }

    if ($action === 'ws_open') {
        classic22LiveWsHandleOpen($archive);
    }

    if ($action === 'ws_send') {
        classic22LiveWsHandleSend($archive);
    }

    if ($action === 'ws_poll') {
        classic22LiveWsHandlePoll($archive);
    }

    if ($action === 'ws_close') {
        classic22LiveWsHandleClose($archive);
    }

    classic22LiveJsonResponse([
        'ok' => 0,
        'message' => 'Unsupported live_action',
    ]);
}

function themeInit($archive)
{
    classic22LiveHandleRequest($archive);
